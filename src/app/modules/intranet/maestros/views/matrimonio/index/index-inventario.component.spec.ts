...
describe('MatrimonioComponent', () => {
  ...
  it('Debe filtrar correctamente con filterMatrimonio', () => {
    ...
    component.filterMatrimonio();

    expect(component.filteredMatrimonio.length).toBe(1);
    expect(component.filteredMatrimonio[0].nombreSolicitante).toBe('Carlos');
  });
  ...
  it('list debe llenar listMatrimonio y filteredMatrimonio', () => {
    const mockResponse = [{ _id: '1', nombreSolicitante: 'Carlos', estado: 1 }];

    const solicitudService = TestBed.inject(SolicitudService);
    (solicitudService.getTramiteList as jasmine.Spy).and.returnValue(
      of(mockResponse),
    );

    component.list();

    expect(component.listMatrimonio.length).toBe(1);
    expect(component.listMatrimonio[0].nombreSolicitante).toBe('Carlos');
    expect(component.filteredMatrimonio).toEqual(component.listMatrimonio);
  });
  ...
  it('debe aprobar correctamente', fakeAsync(() => {
    ...
    const alertService = TestBed.inject(
      AlertService,
    ) as unknown as MockAlertService;
    const solicitudService = TestBed.inject(
      SolicitudService,
    ) as unknown as MockSolicitudService;
